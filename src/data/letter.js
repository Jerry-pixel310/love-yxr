const letterConfig = {
  recipient: '冉冉',
  passphrase: '冉冉宝宝',
  occasion: '520',

  coverTitle: '有一封信',
  coverSubtitle: '只想写给小羊宝宝',
  openButtonText: '打开这封信',

  gatePrompt: '请输入我们之间的小暗号',
  gatePlaceholder: '是那句只有我们懂的话',
  gateSuccessText: '暗号正确，正在把这封信交给你。',
  gateErrorText: '好像还差一点点。\n再想想，是那句很可爱的暗号呀。',

  openingLine: '冉冉宝宝，520 快乐。\n这一次，我想认真又温柔地告诉你一些话。',
  musicButtonText: '播放一点温柔的音乐',
  musicPath: '/music/daoxiang.mp3',

  sections: [
    {
      title: null,
      text: '小羊，其实我一直觉得，\n喜欢你不是一件突然发生的事。\n\n它像是很多个很小很小的瞬间，\n一点一点落在我心里，\n然后慢慢变成了一个很确定的答案。',
    },
    {
      title: null,
      text: '我喜欢和你说话的感觉，\n喜欢你偶尔可爱的语气，\n也喜欢你不经意间让我心软的样子。\n\n有时候只是想到你，\n一天里很普通的时刻，\n也会突然变得亮一点。',
    },
    {
      title: null,
      text: '我不是一个特别会把浪漫说得很漂亮的人，\n但我知道，\n如果很多事情可以重新选择，\n我还是会想靠近你。\n\n想听你讲今天发生的小事，\n想把好吃的分享给你，\n想在你不开心的时候站在你这边。',
    },
    {
      title: null,
      text: '所以在 5 月 20 日这一天，\n我想把这句话认真地放在你面前：\n\n杨星冉宝宝，我喜欢你。\n不是随便说说，\n是很认真、很想一直对你好的那种喜欢。',
    },
  ],

  photosTitle: '以后这里，会放下我们的好多瞬间',
  photos: [
    { src: '/photos/photo-1.jpg', placeholder: '想放一张我很喜欢的你' },
    { src: '/photos/photo-2.jpg', placeholder: '想放一张我们一起开心的瞬间' },
    { src: '/photos/photo-3.jpg', placeholder: '想放一张以后看了还会笑的照片' },
  ],

  reasonsTitle: '喜欢小羊宝宝的 20 个小理由',
  reasons: [
    '喜欢你认真说话的样子。',
    '喜欢你偶尔可爱的小脾气。',
    '喜欢想到你时，心情会变好。',
    '喜欢你让我觉得生活很值得期待。',
    '喜欢你分享日常时的语气。',
    '喜欢你笑起来的时候，好像世界都软了一点。',
    '喜欢你不用刻意，就已经很特别。',
    '喜欢和你聊天时，时间过得很快。',
    '喜欢你让我想把温柔都攒起来给你。',
    '喜欢你出现在我生活里的每一天。',
    '喜欢你可爱，也喜欢你偶尔不那么可爱。',
    '喜欢你让我变得更想认真生活。',
    '喜欢你在我心里越来越重要。',
    '喜欢你让我有了偏爱的理由。',
    '喜欢你的名字出现在我的消息列表里。',
    '喜欢你让我觉得等待也可以很甜。',
    '喜欢你让我想把好东西都留给你。',
    '喜欢你是你，不需要变成别人。',
    '喜欢你让我明白，心动可以很日常。',
    '喜欢你这件事，我想慢慢喜欢很久。',
  ],

  secretEnvelopesTitle: '给冉冉宝宝的 5 个小信封',
  secretEnvelopes: [
    { title: '第一封：想你', text: '有时候不是一定要发生什么，才会想你。只是某个很普通的瞬间，我就会突然想到小羊。' },
    { title: '第二封：偏爱', text: '我想把很多温柔都留给你。不是因为今天是 520，而是因为你是小羊宝宝。' },
    { title: '第三封：陪伴', text: '我希望以后你开心的时候我在，不开心的时候我也在。' },
    { title: '第四封：未来', text: '未来听起来很远，但如果是和杨星冉宝宝一起，好像每一步都值得期待。' },
    { title: '第五封：只给你', text: '这封信、这首歌、这些话，还有我此刻的认真，都只给你。' },
  ],

  futureChecklistTitle: '我们的未来小清单',
  futureChecklist: [
    '一起去看一次海。',
    '一起吃一顿很满足的火锅。',
    '一起散步到很晚很晚。',
    '一起拍很多很多照片。',
    '一起听完一整首《稻香》。',
    '一起在下雨天分享一把伞。',
    '一起去想去的城市。',
    '一起把普通日子过得很甜。',
    '一起过下一个 520。',
    '一起慢慢拥有更多属于我们的回忆。',
  ],

  heartLoadingText: '正在加载我对小羊宝宝的喜欢...',
  heartLoadingResult: '加载失败：喜欢太多，无法计算。',

  chatTitle: '如果把想你写成聊天记录',
  chatMessages: [
    { from: 'me', text: '小羊，在吗？' },
    { from: 'her', text: '怎么啦？' },
    { from: 'me', text: '没什么，就是突然很想告诉你一件事。' },
    { from: 'her', text: '什么事呀？' },
    { from: 'me', text: '今天的风、歌、路灯和晚霞，好像都在提醒我想你。' },
    { from: 'me', text: '还有，520 快乐。' },
    { from: 'her', text: '那你想说的只有这些吗？' },
    { from: 'me', text: '当然不是。后面还有很多很多，都是写给冉冉宝宝的。' },
  ],

  fortuneTitle: '抽一张今天的心动签',
  fortunes: [
    '今日心动签：小羊宝宝会被认真喜欢一整天。',
    '今日幸运事：听到《稻香》的时候，会有人偷偷想你。',
    '今日隐藏奖励：你会收到一份只属于你的温柔。',
    '今日最佳答案：杨星冉宝宝最可爱，这题不用证明。',
    '今日心愿：希望小羊每天都被好好惦记。',
    '今日偏爱值：满格，还溢出来了一点点。',
  ],

  quizTitle: '520 心动小问答',
  quizQuestions: [
    {
      question: '这封信是写给谁的？',
      options: ['路过的人', '冉冉', '风和月亮'],
      answer: '冉冉',
      feedback: '答对啦，这封信从第一行开始就是小羊宝宝的。',
    },
    {
      question: '输入暗号后打开的是？',
      options: ['一封普通网页', '一份只给你的认真', '一个按钮'],
      answer: '一份只给你的认真',
      feedback: '是的，这不是普通网页，是我认真准备的小礼物。',
    },
    {
      question: '我想和小羊一起听完哪首歌？',
      options: ['稻香', '随机播放', '安静'],
      answer: '稻香',
      feedback: '因为这首歌会让温柔慢慢落下来。',
    },
    {
      question: '我对冉冉宝宝的喜欢加载到 100% 后会怎样？',
      options: ['刚刚好', '喜欢太多，无法计算', '自动结束'],
      answer: '喜欢太多，无法计算',
      feedback: '没办法，喜欢杨星冉宝宝这件事很难用数字讲清楚。',
    },
    {
      question: '这份 520 礼物最想告诉小羊宝宝什么？',
      options: ['你很特别', '你值得被好好喜欢', '以上都是'],
      answer: '以上都是',
      feedback: '全都对，小羊就是很特别，也很值得。',
    },
  ],

  capsuleCardTitle: '给未来冉冉宝宝的一颗时间胶囊',
  capsuleCardHint: '点开以后，会收到一封来自今天的温柔。',
  capsuleCardText: '未来某一天，如果小羊宝宝又看到这里，希望你还会记得：在这个 520，有一个人很认真地把喜欢写成网页，把《稻香》放进信里，把很多温柔都藏起来，只等你慢慢发现。',

  starTitle: '点亮 9 颗想你的星星',
  stars: [
    '第一颗星：想把晚安说给你听。',
    '第二颗星：想把好吃的第一口给你。',
    '第三颗星：想陪你走很长的路。',
    '第四颗星：想记住你开心的样子。',
    '第五颗星：想在你累的时候抱抱你。',
    '第六颗星：想和你看很多次日落。',
    '第七颗星：想把平凡日子过成小浪漫。',
    '第八颗星：想做你坚定的偏爱。',
    '第九颗星：想一直一直喜欢你。',
  ],

  loveCardTitle: '生成给杨星冉宝宝的专属情书卡片',
  loveCardLines: [
    'To：小羊宝宝',
    '520 快乐。',
    '今天的歌是《稻香》，今天的信是给你。',
    '我把喜欢写进每一个小卡片里，',
    '也把未来想和你一起做的事，认真放在这里。',
    '愿冉冉宝宝被温柔包围，也被我好好喜欢。',
    'From：稻香',
  ],

  promisesTitle: '还想和小羊一起做很多事',
  promises: [
    '一起吃很多顿饭',
    '一起看一次很漂亮的日落',
    '一起去想去的地方',
    '一起拍很多照片',
    '一起过很多个 520',
    '一直好好喜欢你',
  ],

  finalText: '如果这封信有结尾，\n那我的喜欢没有。',
  finalSubText: '520 快乐，小羊宝宝。\n希望你看到这里的时候，\n能感觉到我很认真地喜欢着你。',

  easterEggButton: '再听我说一句',
  easterEggText: '杨星冉宝宝，你真的很可爱。\n也真的很值得被我好好喜欢。',

  signOff: 'From：稻香',

  spinWheelTitle: '转一转，抽今日约会计划',
  spinWheelItems: [
    '一起去吃很想吃的东西 🍜',
    '一起看一部烂漫爱情电影 🎬',
    '一起散步到很晚 🌙',
    '在家一起做顿饭 🍳',
    '一起去找一家好喝的奶茶 🧋',
    '一起看日落 🌅',
    '一起逛逛书店或者小店 📚',
    '什么都不做，就陪着 🤍',
  ],

  sweetnessTitle: '测测我们的今日甜度',
  sweetnessQuestions: [
    {
      question: '今天有没有想到我？',
      options: ['没有', '偶尔', '经常', '一直都在想'],
      scores: [0, 1, 2, 3],
    },
    {
      question: '如果今天可以做一件事，你想做什么？',
      options: ['睡觉', '出去玩', '和你在一起', '和你一起睡觉'],
      scores: [0, 1, 3, 3],
    },
    {
      question: '看完这封信，你觉得？',
      options: ['还行吧', '挺用心的', '很感动', '我也很喜欢你'],
      scores: [0, 1, 2, 3],
    },
  ],
  sweetnessLevels: [
    { min: 0, max: 2, label: '普通朋友味', desc: '没关系，我继续努力。' },
    { min: 3, max: 5, label: '微甜拿铁', desc: '有一点点心动了？很好。' },
    { min: 6, max: 7, label: '满杯玫瑰奶茶', desc: '小羊真的好甜，我喜欢。' },
    { min: 8, max: 9, label: '超甜限定款！', desc: '这已经是满分了，你真的很可爱。' },
  ],

  timelineTitle: '我们的小时光轴',
  timelineEvents: [
    { date: '遇见你那天', text: '认识了冉冉宝宝，世界好像多了一个颜色。' },
    { date: '第一次聊到很晚', text: '那天我就知道，我很喜欢和你说话。' },
    { date: '慢慢地', text: '你开始出现在我想分享的每一件小事里。' },
    { date: '520 这天', text: '想认真告诉你：我很喜欢你，小羊。' },
    { date: '以后的每一天', text: '这里还没写完，因为故事还在继续。' },
  ],

  countdownTitle: '距离下一个 520',
  countdownSuffix: '还有',

  memoryCinemaTitle: '我们的精彩瞬间',
  memoryCinemaSubtitle: '每一帧都值得被好好保存。',
  memoryPhoto: {
    src: '/images/ranran-memory.webp',
    caption: '把这一刻偷偷收藏起来。',
  },
  starUniverseTitle: '杨星冉的小宇宙',
  starUniverseSubtitle: '听说名字里有星星的人，本来就应该被温柔照亮。',
  starUniverseProgressText: '已点亮',
  starUniverseCompleteTitle: '小羊，星星都亮了。',
  starUniverseCompleteText: '但我还是觉得，杨星冉最亮。',
  starUniversePlanets: [
    {
      id: 'lamb',
      name: '小羊星球',
      emoji: '🐑',
      text: '这里住着一只很可爱的小羊。今天的小羊也要被认真喜欢。',
    },
    {
      id: 'xingran',
      name: '星冉星球',
      emoji: '✨',
      text: '杨星冉，名字里有星星的人，一出现就会让我的世界亮起来。',
    },
    {
      id: 'favorite',
      name: '偏爱星球',
      emoji: '💗',
      text: '在我这里，小羊永远是最特别的选项。',
    },
    {
      id: 'hug',
      name: '抱抱星球',
      emoji: '🤍',
      text: '如果今天有一点点累，那就先收下一个隔空抱抱。',
    },
    {
      id: 'future',
      name: '未来星球',
      emoji: '🌙',
      text: '以后还有好多好多日子，我都想慢慢陪你一起走。',
    },
  ],

  blindBoxTitle: '小羊的今日心动盲盒',
  blindBoxSubtitle: '今天可以打开一份只属于杨星冉的小惊喜。',
  blindBoxButtonText: '打开盲盒',
  blindBoxAgainText: '再开一次',
  blindBoxRewards: [
    {
      title: '今日奖励：隔空抱抱券',
      text: '小羊可以随时兑换一个超级认真抱抱。',
    },
    {
      title: '今日奖励：偏爱体验卡',
      text: '今天的杨星冉也拥有我的第一顺位偏爱。',
    },
    {
      title: '今日奖励：晚安专属券',
      text: '今晚的小羊，可以收下一句只给你的晚安。',
    },
    {
      title: '今日奖励：开心补给包',
      text: '如果今天有一点点不开心，就从这里拿走一点甜。',
    },
    {
      title: '今日奖励：想你提醒',
      text: '叮咚，有人刚刚又偷偷想了你一下。',
    },
    {
      title: '今日奖励：小羊可爱认证',
      text: '认证成功：小羊今日可爱值 100%。',
    },
    {
      title: '今日奖励：星冉幸运签',
      text: '今天适合被喜欢，也适合被温柔对待。',
    },
    {
      title: '恭喜解锁隐藏大奖',
      text: '我真的很喜欢杨星冉。',
    },
  ],

  stampBookTitle: '星冉专属甜甜印章册',
  stampBookSubtitle: '每盖下一枚章，都是我认真喜欢小羊的一点证据。',
  stampBookProgressText: '已收集甜甜印章',
  stampBookCompleteTitle: '小羊，甜甜印章集齐啦。',
  stampBookCompleteText: '隐藏奖励：以后每一天，我都想认真喜欢杨星冉。',
  stampBookStamps: [
    {
      id: 'miss',
      label: '今日想你章',
      mark: '想你',
      text: '叮咚，今天也有好好想杨星冉。',
    },
    {
      id: 'cute',
      label: '小羊可爱章',
      mark: '可爱',
      text: '认证成功：小羊宝宝的可爱没有有效期。',
    },
    {
      id: 'favorite',
      label: '偏爱认证章',
      mark: '偏爱',
      text: '在我这里，星冉永远是特别待遇。',
    },
    {
      id: 'hug',
      label: '抱抱补给章',
      mark: '抱抱',
      text: '如果累了，就从这里取走一个认真抱抱。',
    },
    {
      id: 'lucky',
      label: '星冉幸运章',
      mark: '幸运',
      text: '今天适合被温柔对待，也适合被我喜欢。',
    },
    {
      id: 'always',
      label: '一直喜欢章',
      mark: '一直',
      text: '不是只在今天，是以后很多很多天。',
    },
  ],

  whisperTitle: '写给小羊的悄悄话',
  whisperSubtitle: '每个信封里都藏着一句，只想悄悄说给杨星冉听的话。',
  whisperEnvelopes: [
    {
      id: 'w1',
      color: '#ffb3c1',
      note: '我其实每天都有悄悄想你，只是有时候说不出口。',
    },
    {
      id: 'w2',
      color: '#ffd6a5',
      note: '我喜欢小羊说话的样子，感觉每句话都值得认真听。',
    },
    {
      id: 'w3',
      color: '#b5ead7',
      note: '以后不管发生什么事，我都想先告诉你。',
    },
    {
      id: 'w4',
      color: '#c7ceea',
      note: '你笑的时候我真的觉得，喜欢你这件事太值了。',
    },
    {
      id: 'w5',
      color: '#ffc8dd',
      note: '我有时候想，要是能一直陪着杨星冉就好了。',
    },
  ],

  wishBottleTitle: '给小羊的许愿瓶',
  wishBottleSubtitle: '摇一摇这只装满心愿的小瓶子，看看里面藏着什么。',
  wishBottleShakeText: '摇一摇',
  wishBottleAgainText: '再摇一次',
  wishBottleWishes: [
    {
      id: 'travel',
      emoji: '🗺️',
      wish: '想和小羊一起去一个从没去过的地方，随便去哪都好。',
    },
    {
      id: 'food',
      emoji: '🍜',
      wish: '想和小羊宝宝吃遍所有我们想吃的东西，一家一家慢慢打卡。',
    },
    {
      id: 'morning',
      emoji: '☀️',
      wish: '想有一个早上醒来，第一件事就是和小羊说早安。',
    },
    {
      id: 'rainy',
      emoji: '🌧️',
      wish: '想在下雨天和杨星冉一起窝着，什么都不做也很好。',
    },
    {
      id: 'old',
      emoji: '🌿',
      wish: '想和小羊一起慢慢变老，一直到很久很久以后。',
    },
  ],

  weatherTitle: '小羊今日专属天气预报',
  weatherSubtitle: '每天都有一份只给杨星冉的天气，点击刷新查收。',
  weatherRefreshText: '查收今日天气',
  weatherAgainText: '再刷新一次',
  weatherForecasts: [
    {
      icon: '🌤️',
      condition: '偏爱晴',
      temp: '爱意值 +99°',
      desc: '全天想你，无云，空气里都是小羊宝宝的味道。',
    },
    {
      icon: '🌸',
      condition: '小羊粉',
      temp: '甜度 100%',
      desc: '今天刮的风都是偏爱风，适合被温柔对待。',
    },
    {
      icon: '⭐',
      condition: '星冉晴',
      temp: '幸运值 MAX',
      desc: '今日天气极好，特别适合被喜欢，禁止不开心。',
    },
    {
      icon: '🌈',
      condition: '双彩虹',
      temp: '快乐 × 2',
      desc: '今天开心就好，什么都可以等，开心不能等。',
    },
    {
      icon: '☁️',
      condition: '想你云',
      temp: '思念浓度高',
      desc: '有大片想你云飘过，建议及时补充抱抱。',
    },
    {
      icon: '🌙',
      condition: '晚安月',
      temp: '温柔 99°',
      desc: '今晚月亮很圆，适合想一个叫杨星冉的人。',
    },
  ],

  medalTitle: '星冉的专属勋章墙',
  medalSubtitle: '点亮每一枚勋章，解锁属于杨星冉的专属称号。',
  medalCompleteTitle: '🎖️ 至尊称号解锁：宇宙第一可爱小羊',
  medalCompleteText: '恭喜星冉集齐所有勋章。这世界上只有一个杨星冉，而所有勋章都是为她准备的。',
  medals: [
    { id: 'm1', emoji: '💖', label: '偏爱勋章', hint: '点击领取', desc: '被我偏爱的认证勋章，永久有效。' },
    { id: 'm2', emoji: '🌟', label: '闪闪勋章', hint: '点击领取', desc: '杨星冉发光的样子，任何人都比不了。' },
    { id: 'm3', emoji: '🐑', label: '小羊勋章', hint: '点击领取', desc: '世界上最可爱的小羊，非小羊莫属。' },
    { id: 'm4', emoji: '🍀', label: '幸运勋章', hint: '点击领取', desc: '认识星冉是我这辈子最幸运的事。' },
    { id: 'm5', emoji: '🌙', label: '晚安勋章', hint: '点击领取', desc: '每一个晚安，都是只给小羊的温柔。' },
    { id: 'm6', emoji: '♾️', label: '永远勋章', hint: '点击领取', desc: '不是一会儿，是以后很长很长的时间。' },
  ],

  dailyQTitle: '给小羊的每日一问',
  dailyQSubtitle: '每一道题都是我认真想过的答案，只想说给杨星冉听。',
  dailyQDrawText: '抽一道题',
  dailyQNextText: '再抽一道',
  dailyQRevealText: '查看我的答案',
  dailyQHideText: '收起答案',
  dailyQuestions: [
    {
      q: '如果今天只能做一件事，你希望是什么？',
      a: '陪着杨星冉宝宝，什么都不做也行，待在一起就够了。',
    },
    {
      q: '你觉得我最喜欢你哪一点？',
      a: '是你笑起来的样子。每次看到都觉得喜欢你这件事真的很值。',
    },
    {
      q: '如果可以带你去任何地方，你想去哪里？',
      a: '你想去哪里都可以，只要是和小羊一起，在哪都算好地方。',
    },
    {
      q: '你觉得我们在一起最特别的是什么？',
      a: '就是跟你在一起，我不需要刻意表现，就可以很放松。',
    },
    {
      q: '下一个值得纪念的日子，你想怎么过？',
      a: '想认认真真陪着杨星冉，吃好吃的，然后好好说一次我喜欢你。',
    },
    {
      q: '如果可以给你一个礼物，你最想要什么？',
      a: '最想给的不是东西，是以后每一天都认真在你身边。',
    },
  ],

  shakeTitle: '摇一摇，抽今日运势',
  shakeSubtitle: '手机用户可以直接摇手机，桌面用户点下面的按钮。',
  shakeHint: '摇一摇手机',
  shakeBtnText: '点击摇一摇',
  shakeAgainText: '再摇一次',
  shakeFortunesByStrength: {
    gentle: [
      { emoji: '🌸', title: '轻轻摇，今日甜度：微甜', text: '今天的小羊轻轻被偏爱，像春风一样，不大不小刚刚好。' },
      { emoji: '🍵', title: '轻轻摇，今日运势：平静', text: '今天适合慢下来，想想好多好多开心的事。' },
    ],
    medium: [
      { emoji: '⭐', title: '使劲摇，今日运势：大吉', text: '星冉今天运势超好！适合被喜欢、被偏爱、被温柔对待。' },
      { emoji: '🌈', title: '用力摇，今日甜度：超甜', text: '今天的小羊宝宝甜度爆表，就算什么都不做也很可爱。' },
    ],
    strong: [
      { emoji: '🎉', title: '使劲摇！今日运势：爆发', text: '哇！杨星冉今天的好运已经溢出来了，快接住！' },
      { emoji: '💥', title: '摇太猛了！解锁隐藏运势', text: '隐藏运势已解锁：今天无论发生什么，都有我在。' },
    ],
  },

  replyTitle: '写一句话给我',
  replySubtitle: '小羊想说什么都可以，我会认真回复。',
  replyPlaceholder: '随便写，我都想看……',
  replySendText: '发送',
  replyAgainText: '再写一句',
  replyRules: [
    { keywords: ['想你', '想念', '好想'], emoji: '💗', reply: '我也是，一直都在想你。' },
    { keywords: ['爱你', '喜欢你', '喜欢我'], emoji: '🥰', reply: '我也喜欢你，而且喜欢得很认真。' },
    { keywords: ['开心', '高兴', '快乐', '好玩', '哈哈'], emoji: '🌸', reply: '小羊开心，我就放心了。' },
    { keywords: ['难过', '不开心', '委屈', '哭'], emoji: '🫂', reply: '来，先抱一下。不管什么事，都可以告诉我。' },
    { keywords: ['累了', '好累', '累', '困'], emoji: '🌙', reply: '辛苦了。今天可以什么都不做，好好休息。' },
    { keywords: ['饿', '吃饭', '好饿'], emoji: '🍜', reply: '快去吃东西！小羊宝宝不可以饿着。' },
    { keywords: ['谢谢', '感谢'], emoji: '☀️', reply: '不客气。做这些是因为喜欢你，不需要谢。' },
    { keywords: ['讨厌', '烦'], emoji: '🌈', reply: '哪里不开心了？说给我听，我帮你烦。' },
    { keywords: ['晚安', '睡觉', '睡了'], emoji: '🌙', reply: '晚安，杨星冉。做个好梦，明天也要好好的。' },
    { keywords: ['早安', '早上好', '起床'], emoji: '☀️', reply: '早安，今天也是被我喜欢的一天。' },
  ],
  replyDefault: [
    { emoji: '💌', reply: '我看到了，谢谢小羊愿意跟我说。' },
    { emoji: '🐑', reply: '小羊说的话，我都会认真记住的。' },
    { emoji: '✨', reply: '嗯，我知道了。不管说什么，我都在。' },
  ],

  dragTitle: '把我们的名字拼在一起',
  dragSubtitle: '拖动字块，把"杨星冉"和"杜昊翔"拼到心形里，拼完有惊喜。',
  dragCompleteTitle: '杨星冉 ♡ 杜昊翔',
  dragCompleteText: '就是这样，杨星冉在杜昊翔这里，永远是最重要的那个。',
  dragPieces: [
    { id: 'p1', char: '杨', targetX: 0, targetY: 0 },
    { id: 'p2', char: '星', targetX: 1, targetY: 0 },
    { id: 'p3', char: '冉', targetX: 2, targetY: 0 },
    { id: 'p4', char: '♡', targetX: 1, targetY: 1 },
    { id: 'p5', char: '杜', targetX: 0, targetY: 2 },
    { id: 'p6', char: '昊', targetX: 1, targetY: 2 },
    { id: 'p7', char: '翔', targetX: 2, targetY: 2 },
  ],

  timeCapsuleTitle: '我们的专属时间胶囊',
  timeCapsuleSubtitle: '写一句话，把它封存进时间胶囊。 future 的某天打开，会想起今天的我们。',
  timeCapsulePlaceholder: '想对未来的自己/我们说点什么……',
  timeCapsuleSealText: '封存进胶囊',
  timeCapsuleAgainText: '再写一颗',
  timeCapsuleSealedTitle: '✓ 已封存',
  timeCapsuleSealedText: '这句话已被小心收藏。未来的某天，当你们打开这颗胶囊，会想起今天的心情。',

  missClockTitle: '今天也在想小羊宝宝',
  missClockSubtitle: '从 2026 年 5 月 18 日开始，时间每走一秒，都悄悄变成一句想你。',
  missClockStartDate: '2026-05-18T00:00:00+08:00',
  missClockDateLabel: '今天是',
  missClockSinceLabel: '这是我想小羊的',
  missClockNote: '不是只有特别的日子才想你，是每一个普通的今天，都因为你变得软乎乎。',
  missClockWishes: [
    '愿冉冉宝宝今天被好运轻轻抱住。',
    '愿小羊做题顺顺利利，心情也亮晶晶。',
    '如果风吹到你那里，那是我偷偷寄过去的想念。',
    '今天也要记得：杨星冉真的很棒。',
  ],

  particleUniverseTitle: '小羊宝宝的心动粒子宇宙',
  particleUniverseSubtitle: '把想你变成会动的星星，手指靠近哪里，心动就亮到哪里。',
  particleUniverseHint: '移动鼠标 / 滑动手指，让星星靠近你',
  particleUniverseButtonText: '点一下，换一种心动形状',
  particleUniverseShapes: ['爱心星云', '冉冉', '告白轨道', '星河花束', '小羊星球'],

  iceRoseTitle: '送杨星冉宝宝一朵冰玫瑰',
  iceRoseSubtitle: '把温柔冻成会发光的花，慢慢开在只属于你的夜里。',
  iceRoseNote: '蓝色是想念，粉色是喜欢，碎光都是我藏起来的心动。',

  gaokaoTitle: '给小羊宝宝的高考加油站',
  gaokaoSubtitle: '高三的这一年，你一直在努力。我相信你，也在这里陪你。',
  gaokaoDate: '2025-06-07',
  gaokaoCountdownPrefix: '距离高考还有',
  gaokaoCountdownSuffix: '天',
  gaokaoDayBeforeText: '就是明天了，放轻松，你准备好了。',
  gaokaoTodayText: '今天是高考第一天，深呼吸，相信自己。',
  gaokaoCheers: [
    { emoji: '⭐', text: '星星都在为你加油，我也是。' },
    { emoji: '🌸', text: '不管结果如何，你已经是我的骄傲了。' },
    { emoji: '💪', text: '这一战，是给自己的答卷，不是给别人的。' },
    { emoji: '🌙', text: '累了就歇一歇，我在远处陪你。' },
    { emoji: '✨', text: '你的名字那么好听，一定会出现在录取通知书上。' },
    { emoji: '🐑', text: '小羊最棒了，我一直都知道。' },
  ],
  gaokaoMissText: '虽然不在你身边，但每一天都在认真为你加油。',
  gaokaoExpectText: '希望你带着安心和勇气走进考场，也带着亮晶晶的笑容奔向理想。',

  heartPopupTitle: '把想你拼成一颗心',
  heartPopupSubtitle: '如果喜欢可以被看见，我想让它铺满整个屏幕。',
  heartPopupButtonText: '点亮满屏心动',
  heartPopupCenterTitle: '冉冉宝宝',
  heartPopupCenterText: '我喜欢你',
  heartPopupMessages: [
    '小羊，我想你了',
    '今天也喜欢你',
    '你是我的偏爱',
    '想把温柔都给你',
    '小羊宝宝要天天开心',
    '这颗心只给你',
    '想陪你看很多日落',
    '想听你说好多小事',
    '你一出现世界就亮了',
    '把浪漫都留给你',
    '想一直好好喜欢你',
    '杨星冉宝宝最特别',
  ],
  /* ── 互动特效集 (from Python code) ── */
  effectsShowcaseTitle: '给小羊宝宝的浪漫特效合集',
  effectsShowcaseSubtitle: '这些效果都是专门为你写的程序，现在变成了可以在手机上看的网页版。',

  multiLangHeartTitle: '52 种语言说「我爱你」',
  multiLangHeartSubtitle: '世界上有那么多种语言，每一种都想用来对冉冉宝宝说同一句话。',
  multiLangHeartButtonText: '打开爱心',
  multiLangHeartTexts: [
    'I love you', 'Je t\'aime', 'Ich liebe dich', 'Ti amo', 'Te amo',
    'あいしてる', '사랑해요', '我爱你', '我中意你', '我爱侬',
    '我欢喜你', 'Kocham Cię', 'Я тебя люблю', 'Σ\'αγαπώ', 'Seni seviyorum',
    'Ik hou van je', 'Jag älskar dig', 'Jeg elsker dig', 'Jeg elsker deg',
    'Eu te amo', 'Mahal kita', 'Aku cinta padamu', 'أحبك', 'אני אוהב אותך',
    'ฉันรักคุณ', 'Tôi yêu bạn', 'मैं तुमसे प्यार करता हूँ', 'আমি তোমাকে ভালোবাসি',
    'میں تم سے محبت کرتا ہوں', 'Мен сені сүйемін', 'Мен сени сүйөм',
    'Те сакам', 'Te dua', 'Ljubim te', 'Volim te', 'Szeretlek',
    'Miluju tě', 'Ľúbim ťa', 'Es tevi mīlu', 'Aš tave myliu',
    'Ma armastan sind', 'Minä rakastan sinua', 'Mina armastan sind',
    '勾买蒙', '额爱你', '我待见你', '恩欢喜你', '我爱列',
    '俺喜欢你', '我稀罕你', '阿秋拉嘎', '在一起', '和我交往吧',
    '我喜欢你', '小羊宝宝我爱你',
  ],

  loveDanmakuTitle: '弹幕告白',
  loveDanmakuSubtitle: '把对小羊说的话变成满屏弹幕，让整个世界都听见。',
  loveDanmakuButtonText: '开启弹幕',
  loveDanmakuTexts: [
    '我爱你', '在一起', '我喜欢你', '和我交往吧', '阿秋拉嘎',
    '小羊最可爱', '想你了', '冉冉宝宝', '永远喜欢你', '你是我的全世界',
  ],

  loveQuotePopupsTitle: '此爱不变',
  loveQuotePopupsSubtitle: '每一句话都是认认真真写给杨星冉宝宝的偏爱，点开爱心听我说。',
  loveQuotePopupsButtonText: '打开爱心弹窗',
  loveQuotePopupsTexts: [
    '我们的相遇你遗忘不了',
    '我心里认定是你，那就是你了',
    '还算你眼光不错，最后选了我啊',
    '你的整个存在本身，对我来说都很重要',
    '我当然也是心有所求',
    '程序和结果都不重要，你才重要',
    '我们一定会再次相会',
    '这个世界上如果没有你，该是如何的了无生趣',
    '但是我的心是有偏向的，它偏向你',
    '你是我独特而不同，不可失去的颜色',
    '凡我所能，无有不应',
    '我永远在你身后，而一切会如你所愿',
    '真心是藏不住的',
    '无论是哪里，我都会和你一起去',
    '此爱不变',
    '世界无限',
  ],

  carePopupsTitle: '给小羊宝宝的温柔提醒',
  carePopupsSubtitle: '这些小弹窗，是我每天都想对你说的话。',
  carePopupsButtonText: '收到温柔',
  carePopupsTexts: [
    '多喝水哦', '好好爱自己', '好好吃饭', '保持好心情',
    '我想你了', '顺顺利利', '别熬夜', '天凉了多穿衣服',
    '充实自己', '记得休息', '你很棒的', '有我在呢',
  ],

  heartPetalTrailTitle: '爱心花海',
  heartPetalTrailSubtitle: '一颗流星划过爱心的轨迹，花瓣从光芒中散落，这是我为冉冉宝宝写的浪漫。',
  heartPetalTrailButtonText: '打开花海',
}

export default letterConfig



